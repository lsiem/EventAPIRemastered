/**
 * This is an API used for handleing events across your java based projects.
 * It's meant to be simple to use without sacrificing performance and extensibility.
 * 
 * Currently the API is in alpha phase and alot needs to be fixed/improve. 
 * 
 * If you have any suggestion for improvements/fixes for shit, 
 * feel free to make a pull request on the bitbucket: https://bitbucket.org/DarkMagician6/eventapi/overview.
 * 
 * For information on how to use the API take a look at the wiki:
 * https://bitbucket.org/DarkMagician6/eventapi/wiki/Home
 * 
 * @Todo Set up the wiki.
 */
package com.darkmagician6.eventapi.main;

/**
 * Main class for the API.
 * Currently only contains the version of the API but more will be added later.
 * 
 * @author DarkMagician6
 * @since July 31, 2013
 */
public final class EventAPI {
	/**
	 * The current version of the API.
	 */
	public static final String VERSION = String.format("%s-%s", "0.3", "alpha");

}
